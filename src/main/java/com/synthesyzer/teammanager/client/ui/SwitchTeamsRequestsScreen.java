...
import com.synthesyzer.teammanager.networking.packets.clienttoserver.AcceptTeamSwapRequestPacket;
...
public class SwitchTeamsRequestsScreen extends BaseUIModelScreen<FlowLayout> {

    protected SwitchTeamsRequestsScreen() {
        super(
                FlowLayout.class,
                DataSource.asset(new Identifier(TeamManager.MOD_ID, "switch_teams_screen"))
//                DataSource.file("switch_teams_screen.xml")
        );
    }
    ...
    protected void build(FlowLayout rootComponent) {
        ...
        var players = ScreenUtil.getPlayers(
                ...
                player -> PendingTeamSwapRequests.hasRequestFrom(player.getProfile())
        );

        if (players.size() == 0) {
            rootComponent.childById(VerticalFlowLayout.class, "main-page").child(
                    Components.label(Text.of("No requests to switch teams"))
    ...
    private Component acceptInviteButton(PlayerListEntry player, boolean accept) {
        return Components.button(Text.of(accept ? " ✔ " : " ✕ "), component -> {
                    TMNetwork.CHANNEL.clientHandle().send(
                            new AcceptTeamSwapRequestPacket(player.getProfile().getId(), player.getProfile().getName(), accept)
                    );
                    PendingTeamSwapRequests.deleteRequest(player.getProfile());
                    this.client.setScreen(null);
                })
                .textShadow(true)
                .tooltip(Text.of(accept ? "Accept Request" : "Reject Request"))
